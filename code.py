...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    df = df.resample('M', on=date_col).mean()
    #print(df.head)

    aggregated_value = df.pivot
...
def convert(df,celsius):
    ...
    converted_temp[celsius] =  converted_temp[celsius].apply(lambda x: (x*9/5)+32)
...
categories = categorical(weather)
#print(categories)

numericals = numerical(weather)
...
cloudy_days = clear(weather,'Weather','Cloudy')
#print("Number of cloudy days:", cloudy_days)
...
print(wind_speed_35_vis_25.shape)
...
#monthly_mean_temp = agg_values_ina_month(weather,'Date/Time','Temp (C)','mean')
...
fahrehheit_temp = convert(weather,'Temp (C)')
#print(fahrehheit_temp.head())
fahrenhheit_dewpoint = convert(fahrehheit_temp,'Dew Point Temp (C)')

#print(fahrenhheit_dewpoint.head())
